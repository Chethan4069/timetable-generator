...
const SLOTS_DEFAULT = [1, 2, 3, 4, 5, 6];
...
function EditModal({
  gene, subjects, teachers, rooms,
  days, slots, onSave, onClose, onDelete
}) {
  ...
  const usedDays  = days  || DAYS_DEFAULT;
  const usedSlots = slots || SLOTS_DEFAULT;
      ...
      <div className="bg-gray-800 rounded-2xl p-8 w-full max-w-md
        ...
        <div className="mt-3 pt-3 border-t border-gray-700">
          <button
            onClick={() => {
              if (window.confirm(
                `Delete "${subjects[gene.subject_id]?.name ||
                "this slot"}" from ${gene.day} ` +
                `Slot ${gene.slot_number}?\n\n` +
                `Note: Lab subjects remove both consecutive slots.`
              )) { onDelete(gene); }
            }}
...
function TimetableGrid({
  ...
  electivePairs, colorMap, classInfo,
  config, onGenesChange
}) {
  ...
  // Dynamic slots and days from config
  const SLOTS = config
    ? Array.from(
        { length: config.lectures_per_day },
        (_, i) => i + 1
      )
    : SLOTS_DEFAULT;

  const DAYS = config?.working_days || DAYS_DEFAULT;
  ...
  const handleDeleteSlot = (geneToDelete) => {
    ...
    setSavedId(null);
    toast.success(
      isLab ? "Both lab slots deleted" : "Slot deleted"
    );
  };
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      const pdfColorMap = {
        0: [30,  64,  175], 1: [109, 40,  217],
        2: [4,   120, 87 ], 3: [180, 83,  9  ],
        4: [185, 28,  28 ], 5: [15,  118, 110],
    ...
    <Layout>
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">
          ...
          🗑️ Delete from Edit modal •
          Each tab = one class section
        </p>
        {/* Config info bar */}
        {config && (
          <div className="mt-2 flex gap-4 text-xs text-gray-500">
            <span>⏱️ {config.lecture_duration_mins} min slots</span>
            <span>📚 {config.lectures_per_day} slots/day</span>
            <span>📅 {(config.working_days || []).join(", ")}</span>
          </div>
        )}
      </div>
